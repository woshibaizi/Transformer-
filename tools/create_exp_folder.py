...
def create_exp_folder():
    #Step1:创建run文件夹(如果不存在)
    if not os.path.exists("run"):
        os.mkdir("run")
    #step2:创建train文件夹(if not exist)
    train_folder=os.path.join("run","train")
    if not os.path.exists(train_folder):
        os.mkdir(train_folder)
    #step3:创建exp文件夹(检查是否已经存在)
    ...
    while True:
        ...
        if not os.path.exists(exp_folder):
            ...
            os.mkdir(os.path.join(exp_folder,"weights"))    #创建weights文件夹
            return exp_folder,os.path.join(exp_folder,"weights")#返回exp和weight文件夹路径
...
def create_val_exp_folder():
    ...
    while True:
        ...
        exp_folder=os.path.join(train_folder,exp_folder_name)
        if not os.path.exists(exp_folder):
            os.mkdir(exp_folder)    #创建新的文件夹
            return exp_folder       #返回新的文件夹路径
        exp_num+=1                  #如果文件夹已存在,增加数字,继续查找下一个文件夹
